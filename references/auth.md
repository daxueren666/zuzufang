...
- **登录态 = douban-profile 持久化目录**（存在且非空=有档案）：级别2 用
  `launch_persistent_context(user_data_dir=E:\租房\tools\douban-profile)`，已有
...
- 通道：MediaCrawler（`--lt qrcode`），登录态缓存持久化在
  `E:\租房\tools\MediaCrawler\browser_data\dy_user_data_dir`。
...
位置：`E:\租房\data\auth_state.json`（原子写，UTF-8；auth_common.data_dir() 解析，RENT_ASSIST_DATA 可覆盖；--parallel 下各 worker 写隔离目录不回写主 state）。记录各平台**最近一次实测**
