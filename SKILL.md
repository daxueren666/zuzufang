...
数据流：run_collect.py 批次编排采集脚本（opencli 小红书 / 豆瓣 HTTP / Exa+Jina / 抖音 MediaCrawler；12315 不进编排器单独跑）→ `E:\租房\data\raw\*.jsonl`
→ `clean.py` → `E:\租房\data\cleaned\*.json` → **Claude 只读 cleaned 做语义分析** → `E:\租房\data\analysis\<标的>.json`（按标的命名，防多标的互相覆盖）
→ `render.py` → `E:\租房\data\reports\*.html`。
...
`<skill>` 指本 skill 目录。数据目录：E:\租房\data（环境变量 RENT_ASSIST_DATA 可覆盖，已 setx；旧 ~/.rent-assist/data 仅作回退；skill 目录保持可分发干净），raw/cleaned/analysis/geo/reports/media、缓存 cache.db、批次进度账本 progress/（断点续采与 summary 都在这里）均在 data 目录下，不落 skill 目录。按输出处理：
...
| 意图 | 判断规则 | 示例问法 |
|---|---|---|
| A 小区综合了解 | 有明确标的（小区/公寓/房东/中介名），泛了解"它怎么样"：口碑、价格、房源、出租方，用户问什么就查什么 | "天通苑怎么样""天通苑租金多少钱""天通苑房东直租靠谱吗""这家中介靠谱吗" |
| B 推荐评价 | 有明确标的 + 明确求推荐/值不值/对比的决策判断 | "天通苑租房推荐吗""XX公寓值得租吗""天通苑和回龙观选哪个" |
| C 选址建议 | 只有工作地/预算/通勤要求，无明确标的，问住哪 | "在国贸上班预算3000住哪合适""望京上班哪个性价比高" |
...
2. **下钻流程**：
   ...
   - **合并清洗**：逐子小区调用 `python <skill>/scripts/clean.py --query <子小区名>`（如 `--query "天通苑本三区"`），产出各自 `E:\租房\data\cleaned\<子小区>.json` 后合并进同一次分析。
...
4. **批次采集（一条 run_collect 命令，编排器自动多轮累计；脚本内部请求间隔随机、批间休眠 10-30s；评论一律按点赞最热排序取 top；时间窗与排序按下方硬规则传参）**：
   ...
   - **语义**：外层遍历查询词、内层遍历平台，每个（词 × 平台）组合一批（每批每平台 limit 20），自动累计到 `--target` 总量（`--per-platform N` 可直接定每平台上限）；断点续采：中断后重跑自动跳过已完成组合（进度账本 E:\租房\data\progress\<slug>.json，done 超 7 天自动失效）。
...
5. **清洗**：`python <skill>/scripts/clean.py --query <标的>` → 产出 `E:\租房\data\cleaned\<标的>.json`（去重、广告过滤、求租帖剔除、8 类风险粗分类、房源帖识别、评论取 top）。
6. **媒体处理（用户 2026-08-15 定：小红书不读图）**：xhs 以正文+评论为主，**不默认下载图片识别**；fetch_media.py 保留为可选工具，仅在用户点名"看看帖子里的图"时使用（`--note-ids "id1,id2"` 或完整 URL）。
7. **抖音视频口播转写**：采集命令加 `--get-video N`（N≤3，对讨论 top 视频帖：MediaCrawler 下载→ffmpeg 抽音频→sherpa-onnx+SenseVoice 本地转写→文本以"【口播转写】"并入该帖 content，extra.asr=true）。注意：MediaCrawler 开视频后会下载搜索页全部结果，**--get-video 务必配小 limit（如 --limit 3）**控带宽。运行时已装 E 盘（E:\租房\tools\asr-venv + models，缺依赖时脚本 exit 3 给指引）；手动转写单文件用 `python <skill>/scripts/asr.py --video <文件>`。
8. **语义分析（Claude 亲自做，只读 E:\租房\data\cleaned\*.json）**：首要任务是**回答用户提出的那个问题**：用户问推荐就给推荐结论，问价格就汇总价格锚点，问二房东就查二房东证据。references/risk-signals.md 的风险八维只作为组织发现的分类词汇表（复核 clean.py 粗分类、剔除否定语境误命中如"从来没漏过水"），不是分析的目的。按下方 schema 写出 `E:\租房\data\analysis\<标的>.json`（按标的命名，防多标的互相覆盖；schema 不变）。
9. **渲染**：`python <skill>/scripts/render.py --analysis E:\租房\data\analysis\<标的>.json --cleaned E:\租房\data\cleaned\<标的>.json`（启用地理层时再加 `--geo E:\租房\data\geo\<标的>.json`）→ `E:\租房\data\reports\<标的>_<YYYYMMDD>.html`。最后告知用户报告绝对路径，并给一句话结论。
...
2. **FAQ 可视化报告**：把答案整理成 `mode = "faq"` 的 analysis.json，跑 render.py 出报告落 `E:\租房\data\reports\`：
...
4. **组装** `E:\租房\data\geo\<标的>.json`：四段输出拼成 `{"geocode":{location,formatted_address,city},"around":{"radius_m":1500,"groups":[{group,pois:[{name,distance_m,address,location}]}]},"noise":{"total":0,"hits":[{type,name,distance_m,address,location}]},"route":{mode,summary,distance_m,duration_min,transfers,origin_location,dest_location}}`，可参照现有 `E:\租房\data\geo\天通苑.json`。
...
6. **渲染**：`python <skill>/scripts/render.py --analysis E:\租房\data\analysis\<标的>.json --cleaned E:\租房\data\cleaned\<标的>.json --geo E:\租房\data\geo\<标的>.json`。模板渲染地图块（小区强调色 marker、配套灰点；仅当 geo.json 带 noise 段才画噪音红点，仅当 coverage.note 标了「通勤目的地：」才画通勤虚线）+ 断网可读的文字层（配套分组网格；通勤摘要仅当用户提供了目的地）。自定义组与预置组同构（group=关键词、pois 同字段），模板按组名直接显示，无需改动。
7. **密钥**：`E:\租房\config\keys.env` 的 `AMAP_JSAPI_KEY`（进模板加载地图，客户端可见属正常，靠高德控制台域名白名单保护）与 `AMAP_JSAPI_SECRET`（securityJsCode）。`AMAP_JSAPI_SECRET` 待填时地图自动降级为文字版，填好后无需改模板。`AMAP_WEB_KEY` 只供 geocode.py 本地调用，**绝不进报告**。
...
| run_collect.py | `--per-platform N`（每平台上限，推荐，消除总量歧义）或 `--target`（总量停止线）；`--queries "Q1,Q2" --platforms "xhs,douban,web,douyin" --days --sort "discussion,hot" --min-per-platform [--extra-queries --batch-limit --top-comments 20 --parallel --douban-intent word\|listing --refresh --reuse-min N]`。`--parallel` 分平台并行子进程（各平台独立账本/日志，默认串行）；7 天复用闸门自动跳过已采组合，`--refresh` 强制重采 | E:\租房\data\raw\*.jsonl + progress/<slug>.summary.json |
| collect_xhs.py | `--query --limit --top-comments --days --sort discussion\|hot\|time（可逗号组合）`；note/comments 失败自动重试 1 次 | E:\租房\data\raw\xhs_<标的slug>_<日期>.jsonl |
| collect_douyin.py | `--query --limit --top-comments --days --sort discussion\|hot\|time（可逗号组合） --get-video N`（N≤3 下载 top 视频并口播转写，须配小 limit） | E:\租房\data\raw\douyin_*.jsonl |
| collect_douban.py | `--query --limit --top-comments --days --sort --intent word\|listing`（口碑=word 裸搜评价词；找房=listing 拼租赁词）`--rental-words --group` | E:\租房\data\raw\douban_*.jsonl |
| collect_web.py | `--query（可传多次） --limit --days --sort discussion\|hot\|time（可逗号组合）`；Exa 失败自动重试、Jina 失败降级直连再降级摘要 | E:\租房\data\raw\web_*.jsonl |
| collect_12315.py | `--query --limit --days` | E:\租房\data\raw\complaint12315_*.jsonl |
| ensure_auth.py | `--platform xhs\|douban\|douyin\|all` | 子脚本退出码 3 后先跑它恢复登录态（douban 弹浏览器轮询等扫码，绝不秒退） |
| fetch_media.py | `--note-ids "id1,id2"`（自动回 raw 查签名 URL）或直接传完整 URL；`--max-images-per-note 5` | E:\租房\data\media/<note_id>/（图片已压缩，供 Claude 视觉读图） |
| asr.py | `--video <文件>` 或 `--video-dir <目录>`（sherpa-onnx+SenseVoice int8，已装 E:\租房\tools\asr-venv+models；缺依赖 exit 3 带指引） | 同名 .txt 转写文本（自动 ffmpeg 抽 16k wav） |
| clean.py | `--query <标的>`（必填）；可选 `--raw-dir --out --aliases --city --max-age-months --no-entity-filter --keep-seek`（`--city <城市>` 城市消歧：黑名单 72 城+北京信号白名单，剔外地同名小区帖，meta 计 city_mismatch/city_kept_signal；默认剔除求租帖并计 meta.seek_posts） | E:\租房\data\cleaned\<标的>.json |
| render.py | `--analysis E:\租房\data\analysis\<标的>.json [--cleaned E:\租房\data\cleaned\<标的>.json] [--geo E:\租房\data\geo\<标的>.json] [--out]` | E:\租房\data\reports\<标的>_<日期>.html |
...
- **用户入口 = 双击 `E:\租房\启动租房助手.bat`**：起本地服务（8770，仅 127.0.0.1）并自动打开浏览器门面页（templates/landing.html，蓝白风+SVG楼群动画，手机/桌面双端适配）。
- 门面页输入任意租房需求 → 点"开始查询" → 服务拉起新终端窗口跑 `claude "<需求>"`（在 E:\租房 项目目录内，权限生效；量级菜单等交互在该终端里完成）→ 服务每 5s 盯 data/reports，新报告生成后页面自动弹出。
...
- 位置：`E:\租房\tools\MediaCrawler`（本地部署，要求 Python ≥3.11 与 Node.js ≥16，用其 `.venv` 运行）。
...
- Playwright 内核装在 E 盘：`PLAYWRIGHT_BROWSERS_PATH=E:\租房\tools\playwright-browsers`（不设则 playwright 找不到 chromium）。collect_douyin.py 启动 MediaCrawler 时已自动注入，手动跑 MediaCrawler 时需自己 export。

## Token 纪律（铁律）

1. **永不读取 E:\租房\data\raw\*.jsonl**，原始数据只供脚本消费。
...
- 同标的 7 天内复用 E:\租房\data\raw 已有数据，不重复采集。
...
- 根因结论记进 E:\租房\终测记录.md 留痕（例：r.jina.ai 免费无 key 走共享池约 20 RPM 不稳，加免费 key 后独立配额约 200 RPM）。

## 密钥安全

- 高德密钥只放 `E:\租房\config\keys.env`（数据盘，skill 目录外；旧 ~/.rent-assist/keys.env 仅回退兼容）；**绝不**放进 skill 目录、报告、缓存或任何仓库。
- `AMAP_WEB_KEY` 只供 geocode.py 本地调用 REST，禁止出现在任何 HTML/日志/输出；`AMAP_JSAPI_KEY` 仅允许出现在 reports/*.html（前端加载地图必需）。
- 分发/拷贝 skill 时默认不含任何 key，接收方需自建 `E:\租房\config\keys.env`（申请方法见 references/amap-api.md）。
