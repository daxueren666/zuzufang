...
2. **搜索词 = 用户原话组合（不生成候选、不预设范围）**：`{工作地}上班 住哪`、`{工作地} 通勤 租房`、`{工作地} 租房推荐` 等 2-3 组（口碑视角，"租房"泛词不单独用）；一条编排直接采：`python <skill>/scripts/run_collect.py --target 60 --queries "{工作地}上班住哪,{工作地}通勤租房" --platforms "xhs,web,douyin,douban" --days 180 --sort "discussion,hot"`（不跑 12315）。用户输入已含意向地名的照 A/C 分界走 A。
