...
2. **搜索词 = 用户问法 + 分平台词形（不生成候选、不预设范围）**。原则（问题记录_v3 #1 定调）：**格式转换 ≠ 自作主张**——把用户的人话拆成关键词是翻译，红线是改意思（加没说的限定词、换标的、生成片区）。2-3 组词里问法词与关键词组混搭，各平台各取所需：
   - 问法词组（web 语义搜索整句直接懂；xhs/douyin 搜索半语义、问法帖是真实体裁）：`{工作地}上班住哪`、`{工作地} 通勤 租房`
   - 关键词组（关键词平台的正路， douban 裸词见 references/queries.md §1.5）：`{城市前缀}{工作地} 租房推荐`、`{工作地} 怎么样`
