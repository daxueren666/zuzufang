...
3. **量级确认（采集前必做）**：给用户三档菜单让其选择（可自定义数字）：
   ...
   耗时依据实测约 2.5 条/分钟/平台（串行）；加 `--parallel` 四平台并行约减半。用户不选就默认**轻量档**（60 条约 20 分钟——采集成本承诺最小，数据不够按下限表规则补采，不起步就重采）。**前端衔接（主动拉起 + 队列直达闭环）**：量级确认前先探测门面页——`curl -s http://127.0.0.1:8770/ping` 返回 `"ok":true` 即服务已在跑（`alive` 只反映心跳，false 时仅补挂 Monitor，**不要重复启动**）；没起则后台以直达模式启动 `RENT_ASSIST_INBOX=1 python <skill>/scripts/serve_home.py`（run_in_background，电脑浏览器自动弹出），**并挂 Monitor 接收队列**（touch `<data>/inbox/heartbeat` 起 5 秒心跳循环 + `tail -n 0 -f <data>/inbox/queue.jsonl`，队列每行即一条页面提交的命令，按本 skill 工作流处理）。把启动横幅里「手机访问(需同一WiFi): http://<局域网IP>:8770/」转述给用户。闭环保证：页面提交 → 本会话接收处理 → 报告落 `<data>/reports/` → serve_home 盯到新报告**自动在原页面弹出**（电脑/手机同享）。页面提交自带【量级：X】标注（X 含档名与条数），Claude 不再重复追问量级；**勿重复启动**（8770 双实例会抢占端口）。
...
2. 客观维度按用户问的来：用户提了交通/通勤才评估交通配套（依据帖子中的地铁/商圈信息，标注"未实地验证"），没提就不自加维度（零假设铁律同样适用于评估维度）。
3. `verdict` 字段必须是一句话结论，**只在用户明说了约束时附适用人群**（例：用户说了"通勤优先"才写"通勤优先者可考虑"），没说约束就只给结论不给人群画像，不给绝对化承诺。
...
1. **信息确认**：工作地（地标/地铁站）、预算、可接受通勤时长、其他要求（合租/整租、电梯房等），缺则一次问齐；用户拒绝答或说"随便"就按未指定继续，不阻塞。
2. **搜索词 = 用户原话组合（不生成候选、不预设范围）**：`{工作地}上班 住哪`、`{工作地} 通勤 租房`、`{工作地} 租房推荐` 等 2-3 组（口碑视角，"租房"泛词不单独用）；一条编排直接采：`python <skill>/scripts/run_collect.py --target 60 --queries "{工作地}上班住哪,{工作地}通勤租房" --platforms "xhs,web,douyin,douban" --days 180 --sort "discussion,hot"`（量级按第 3 步菜单定，示例为轻量档；不跑 12315）。用户输入已含意向地名的照 A/C 分界走 A。
3. **候选片区从清洗后数据归纳**：Claude 统计帖子里实际被推荐/讨论的居住片区，candidates 每条必带 evidence（引文+原帖链接），被推荐频次高、口碑一致者优先；数据里有几个写几个（如"中关村 2 小时通勤"数据可能覆盖全城，就照数据写）。
4. **通勤分析**：对数据归纳出的候选做通勤测算（起点=候选片区、终点=用户工作地，用户给了工作地才跑）；M3 之前文本估算（标注"文本估算"），之后 `geocode.py` route 实测。通勤信息只进报告的通勤区块，**绝不进平台搜索词**。
5. 汇总写 `mode = "locate"` 的 analysis.json（candidates：name/pros/cons/commute/evidence + 口碑风险摘要）→ render → 输出数据归纳的片区排名 + 提醒实地看房。
